...
class GossipTransportTest {
    ...
    val unsignedJson = toJsonString(jo)
    val canonical = ResourceOfferVerifier.canonicalizeForSigningString(unsignedJson)
    ...
    val transport = GossipTransport(incomingSignedJsonFlow = flow, scope = kotlinx.coroutines.CoroutineScope(kotlinx.coroutines.Dispatchers.Unconfined))
    val client = MeshrabiyaClient.Builder().withTransport(transport).trustStore(org.abhaya.sensor.meshrabiya.TrustStore(null)).build()

    // Emit synchronously to ensure transport collector sees the event reliably in tests
    flow.emit(toJsonString(signed))
    ...
    flow.emit(toJsonString(tampered))
